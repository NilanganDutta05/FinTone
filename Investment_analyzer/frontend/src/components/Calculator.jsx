...
const ResultCard = ({ label, value, colorClass = "text-white" }) => (
  <div className="bg-slate-800/40 border border-slate-700/50 p-5 rounded-2xl backdrop-blur-sm relative overflow-hidden group hover:border-slate-600 transition-all">
    ...
    <p
      className={`text-3xl font-bold tracking-tight relative z-10 ${colorClass}`}
    >
      {value}
    </p>
  </div>
...
const Calculator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/analyze-scheme`, {
        method: "POST",
        headers: {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <InputField
                ...
                name="premium_per_year"
                value={formData.premium_per_year}
              ...
              <InputField
                label="Premium Years"
                name="premium_years"
                value={formData.premium_years}
                onChange={handleChange}
              />
              <InputField
                label="Payout Every (Years)"
                name="payout_every"
                value={formData.payout_every}
                onChange={handleChange}
              />
              <InputField
                label="Payout Amount"
        ...
        {result ? (
          <div className="glass-panel rounded-3xl p-8 border-t border-t-purple-500/30 animate-in fade-in slide-in-from-bottom-8 duration-500 h-full flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ResultCard
                ...
                value={`₹${result.analysis.total_invested.toLocaleString()}`}
              />
              <ResultCard
                label="Total Return"
                value={`₹${result.analysis.total_return.toLocaleString()}`}
          ...
          <div className="glass-panel rounded-3xl p-8 border-dashed border-2 border-slate-700/50 h-full flex flex-col items-center justify-center text-center text-slate-500">
            ...
            <p className="text-sm max-w-[250px]">
              Enter your investment details and hit calculate to generate
              precision analytics.
            </p>
          </div>
        )}
